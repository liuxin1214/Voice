...
async def wrapper(ctx, method="post", apiName=None, logic=None, containResponseMetadata=True):
    if ctx.method.lower() == method and ctx.url.startswith(f"/{apiName}"):
        metadata = {"Action": apiName}
        try:
            result = await invoke_logic(logic)
            ctx.body = {"ResponseMetadata": metadata, "Result": result} if containResponseMetadata else result
        except Exception as exc:
            ctx.body = {"ResponseMetadata": {**metadata, "Error": {"Code": -1, "Message": str(exc)}}}
