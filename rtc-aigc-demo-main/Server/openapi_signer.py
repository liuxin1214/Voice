...
def sign_request(method: str, host: str, path: str, query: dict, body: bytes,
                 access_key: str, secret_key: str, region: str = "cn-north-1",
                 service: str = "rtc", now: datetime | None = None) -> dict[str, str]:
    ...
    canonical_request = "\n".join([method.upper(), path or "/", canonical_query, canonical_headers, signed, _hash(body)])
    scope = f"{date}/{region}/{service}/request"
    string_to_sign = "\n".join(["HMAC-SHA256", timestamp, scope, _hash(canonical_request)])
    ...
    return {"Host": host, "Content-type": "application/json", "X-Date": timestamp, "Authorization": authorization}
